...
<script type="text/javascript">
    ...
    function save_data() {
        if (!$('[name=users_username]').val() || !$('[name=users_name]').val()) {
            alert("Maaf ada field yang wajib di isi, mohon cek kembali");

            $("#error_name").addClass('text-red');
            $("#error_name").css("display", "block");

            $("#error_username").addClass('text-red');
            $("#error_username").css("display", "block");
        } else {
            if ($('[name=users_password]').val() !== $('[name=users_passwordconfirm]').val()) {
                alert('Password tidak sama dengan password konfirmasi, silahkan coba lagi');
            } else {
                $.ajax({
                    ...
                    success: function () {
                        ...
                        $("#data").load('<?php echo base_url('users/show/#data'); ?>');
                    }
                });
            }
        }
    ...
    function modal_data(mode, id_param) {
        ...
        var title = 'Tambah';
        $.ajax({
            url: "<?php echo base_url(); ?>users/modal/" + mode + '/' + id,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                $(".modal-body").load("<?php echo base_url(); ?>users/modal/" + mode + '/' + id);
            }
        });
    }
