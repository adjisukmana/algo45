...
<script type="text/javascript">
    function load_data() {
        $.ajax({
            url: "<?php echo site_url('master/product_rule'); ?>",
            beforeSend: function () {
    ...
    function save_data() {
        if (!$('[name=product_rule_set]').val()) {
            alert("Maaf ada field yang wajib di isi, mohon cek kembali");

            $("#error_set").addClass('text-red');
            $("#error_set").css("display", "block");
        } else {
            $.ajax({
                url: "<?php echo base_url('master/product_rule/save'); ?>",
                data: $("form").serialize(),
                type: "POST",
                success: function () {
                    $('#myModal').modal('toggle');
                    $("#data").load('<?php echo base_url('master/product_rule/show/#data'); ?>');
                }
            });
        }
    }
    ...
    function saved_data() {
        if ($(":checkbox:checked").length === 0) {
            ...
            $.ajax({
                url: "<?php echo site_url('master/product_rule/saved'); ?>",
                data: $("form").serialize(),
                type: "POST",
                beforeSend: function () {
                    $("#data").html("<img src='<?php echo base_url($this->config->item('resources_dir') . $this->config->item('resources_back') . 'img/ajax-loading.gif'); ?>' class='img-responsive' style='margin: 10px auto;'/>");
    ...
    function modal_data(mode, id_param) {
        ...
        var title = 'Tambah';
        $.ajax({
            url: "<?php echo base_url(); ?>master/product_rule/modal/" + mode + '/' + id,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                $(".modal-body").load("<?php echo base_url(); ?>master/product_rule/modal/" + mode + '/' + id);
            }
        });
    }
